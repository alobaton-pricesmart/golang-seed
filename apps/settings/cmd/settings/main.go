...
import (
	...
	"github.com/alobaton/golang-seed/apps/settings/pkg/services/settings"
	"github.com/alobaton/golang-seed/pkg/services"
)

func registerRoutes(r *httprouter.Router) {
	server := settings.NewSettingsServer()

	r.POST("/", server.CreateSetting)
}
...
func main() {
	...
	service := services.Init("settings")
